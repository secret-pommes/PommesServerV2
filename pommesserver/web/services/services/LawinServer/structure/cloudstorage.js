...
express.use((req, res, next) => {
    ...
    if (req.originalUrl.toLowerCase().startsWith("/lawinserver/fortnite/api/cloudstorage/user/") && req.method == "PUT") {
        req.rawBody = "";
...
express.get("/lawinserver/fortnite/api/cloudstorage/system", async (req, res) => {
...
express.get("/lawinserver/fortnite/api/cloudstorage/system/:file", async (req, res) => {
...
express.get("/lawinserver/fortnite/api/cloudstorage/user/*/:file", async (req, res) => {
    try {
...
express.get("/lawinserver/fortnite/api/cloudstorage/user/:accountId", async (req, res) => {
    try {
        if (!fs.existsSync(path.join(process.env.LOCALAPPDATA, "LawinServer", "ClientSettings"))) {
            fs.mkdirSync(path.join(process.env.LOCALAPPDATA, "LawinServer", "ClientSettings"));
        }
...
express.put("/lawinserver/fortnite/api/cloudstorage/user/*/:file", async (req, res) => {
    try {
        if (!fs.existsSync(path.join(process.env.LOCALAPPDATA, "LawinServer", "ClientSettings"))) {
            fs.mkdirSync(path.join(process.env.LOCALAPPDATA, "LawinServer", "ClientSettings"));
        }
